...
class TestStepGenerator:
    ...
    def _generate_steps_for_test_case(self, test_case: TestCase, sttm_tab: STTMTab, 
        ...
        for modified_field in change_info.get('modified_fields', []):
            modification_step = self._generate_modification_for_field(
                test_case, modified_field, assessment
            )
            if modification_step:
                generated_steps.append(modification_step)
    ...
    def _generate_modification_for_field(self, test_case: TestCase, modified_field: str,
                                       assessment: TestCaseImpactAssessment) -> Optional[GeneratedTestStep]:
        """Generate modification for an existing step that references a modified field"""
        
        # Find the best step to modify
        step_to_modify = self.reference_finder.find_steps_for_modification(test_case, modified_field)
        
        if not step_to_modify:
            self.logger.debug(f"No existing step found to modify for field '{modified_field}'")
            return None
