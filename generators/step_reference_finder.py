...
class StepReferenceFinder:
    ...
    def _step_references_field(self, step: TestStep, field_name: str) -> bool:
        """Check if a test step references a specific field name"""
        if not field_name or not field_name.strip():
            return False
        ...
        # Use word boundary matching for exact field names
        pattern = r'\b' + re.escape(field_name) + r'\b'
        
        # Case-insensitive search
        return bool(re.search(pattern, step_text, re.IGNORECASE))
    
    def _get_field_match_locations(self, step: TestStep, field_name: str) -> List[str]:
        """Get locations where field name is found in the step"""
        locations = []
        pattern = r'\b' + re.escape(field_name) + r'\b'
        ...
        if step.expected_result and re.search(pattern, step.expected_result, re.IGNORECASE):
            locations.append('expected_result')
        
        return locations
    
    def find_steps_for_modification(self, test_case: TestCase, modified_field: str) -> Optional[Dict[str, Any]]:
        """Find the best step to modify for a changed field"""
        candidates = self.find_steps_referencing_field(test_case, modified_field)
        
        if not candidates:
            return None
        
        # If multiple steps reference the field, prefer the one with more specific references
        if len(candidates) == 1:
            return candidates[0]
        
        # Score candidates based on reference quality
        best_candidate = None
        best_score = -1
        
        for candidate in candidates:
            score = self._calculate_modification_score(candidate, modified_field)
            if score > best_score:
                best_score = score
